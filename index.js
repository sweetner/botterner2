...
bot.on("message", function(message) {
  ...
  if (!XP[message.author.id]) 
   XP[message.author.id] = {
	...
	let userXP = XP[message.author.id].XP ? XP[message.author.id].XP : 0;
  ...
  {
      XP[message.author.id].cooldown = 2;
      setTimeout (function () { 
      XP[message.author.id].cooldown = 0;
      },1000 * 60); 
  }
  else if (XP[message.author.id].cooldown == 0)
  {
    XP[message.author.id].XP += 20;
    XP[message.author.id].cooldown = 1;
...
case "love":
    ...
    if ((lovedmention[0].id == 290154978742632449 && lovedmention[1].id == 109378124898136064) || (lovedmention[1].id == 290154978742632449 && lovedmention[0].id == 109378124898136064))
  ...
  case "lvl":
  case "level":
      let mentionedLevel = message.mentions.users.first();
      if (!mentionedLevel) return message.reply("you are currently level **"+ XP[message.author.id].level+"**" );
      message.channel.send(mentionedLevel +" is level " + "**"+ XP[mentionedLevel.id].level + "**");
      break;
  case "setlevel":
  case "setlvl":
      if (message.author.id == 109378124898136064)
      {
        ...
        XP[message.mentions.users.first().id].level = args [2];
        fs.writeFile("./XP.json", JSON.stringify(XP), (err) => {
          if (err) console.error(err)
          });
      }
  ...
  case "nick":
  case "nickname":
      var userHasPerm = message.guild.member(message.author).hasPermission("MANAGE_NICKNAMES",false, true, true) ? 1 : 0;
      if (!message.mentions.users.first()) 
        return message.reply("Please mention a user!");
      let nick = message.content.substring(PREFIX.length + args[0].length + args[1].length + 1);
      if (userHasPerm || message.author.id == 109378124898136064 || message.author.id == 290154978742632449)
        message.guild.member(message.mentions.users.first()).setNickname(nick);
      else
